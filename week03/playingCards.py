...
def find_available_card(deck_of_cards):
    ...
    tmp_list = [card for card, owner in deck_of_cards.items()
                if owner == 'pile']
    return tmp_list[0]
...
def print_hand(deck_of_cards, players):
    ...
    for player in players:
        ...
        print_hand_for_player(deck_of_cards, player)


def print_hand_for_player(deck_of_cards, player):
    """Function to print out hand for a certain player."""
    print({card for card, owner in deck_of_cards.items()
           if owner == player})
...
if __name__ == '__main__':
    ...
    player = 'matt'
    print("What cards are does %s the have?" % player)
    print_hand_for_player(my_deck, player)
